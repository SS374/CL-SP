...
def predict(datasets):
    ...
    predictionlist = [
        None for x in range(len(last_dataset))
    ]  # Placeholer list for prediction dataset
    trend = findtrend(datasets)

    if (
        "same" in trend
    ):  # If all previous data was identical, assume prediction will be identical
        return (
            "["
        ...
        weightedPercentages = (
            []
        )  # Stores deviance percentage for different index locations
        for i in range(
            len(last_dataset)
        ):  # Looks at each index of datasets, only uses values at the same index location to form prediction
    ...
    if deviance == 0 or weightedPercentage == 0:
        ...
        deviance > 0
        and deviance <= 1
        or weightedPercentage > 0
        and weightedPercentage <= 10
    ):
        confidence = "High"
    elif (
        deviance > 1
        and deviance <= 4
        or weightedPercentage > 10
        and weightedPercentage <= 20
    ):
        confidence = "Moderate"
    elif (
        deviance > 4
        and deviance <= 6
        or weightedPercentage > 20
        and weightedPercentage <= 30
    ):
        confidence = "Moderately Low"
    elif (
        deviance > 6
        and deviance <= 7
        or weightedPercentage > 30
        and weightedPercentage <= 50
    ):
        confidence = "Low"
    else:
